...
const EVENTS_VEC_CAPACITY: usize = 2 * MAX_NO_OF_SPANS + MAX_NO_OF_EVENTS;
...
impl GuestState {
    ...
    pub(crate) fn guest_trace_info(&mut self) -> TraceBatchInfo {
        let serialized_data: Vec<u8> = (&self.data).try_into().unwrap();
    ...
    pub(crate) fn record(&mut self, s_id: &Id, values: &Record<'_>) {
        ...
        if let Some(GuestEvent::OpenSpan {
            id: _,
            parent_id: _,
            name: _,
            target: _,
            tsc: _,
            fields,
        }) = spans.iter_mut().find(|e| {
            if let GuestEvent::OpenSpan {
                id,
                parent_id: _,
                name: _,
                target: _,
