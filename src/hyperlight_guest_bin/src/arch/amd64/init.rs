...
unsafe fn init_tss(pc: *mut ProcCtrl) {
    unsafe {
        ...
        // The exception stack (IST1) grows downward. Place its top
        // immediately below the metadata and clock pages at the top of
        // scratch so page-fault / COW handlers never clobber the
        // hypervisor-owned clock page or the bookkeeping data; the stack
        // then runs down through its own two reserved pages.
