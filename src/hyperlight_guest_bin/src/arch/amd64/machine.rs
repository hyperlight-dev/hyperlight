...
use hyperlight_common::vm::PAGE_SIZE;
use hyperlight_guest::layout::arch::PROC_CONTROL_GVA;
use core::mem;
...
const _: () = assert!(mem::size_of::<ProcCtrl>() <= hyperlight_common::vm::PAGE_SIZE * 2);
...
impl ProcCtrl {
    ...
    pub(super) unsafe fn init() -> *mut Self {
        unsafe {
            let ptr = hyperlight_guest::layout::arch::PROC_CONTROL_GVA as *mut u8;
            crate::paging::map_region(
                hyperlight_guest::prim_alloc::alloc_phys_pages(2),
                ptr,
                hyperlight_common::vm::PAGE_SIZE as u64 * 2,
            );
