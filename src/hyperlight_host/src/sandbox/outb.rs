...
pub enum HandleOutbError {
    /// Stack overflow detected (signaled by guest)
    StackOverflow,
    /// Guest aborted: error code {code}, message: {message}
    GuestAborted {
    ...
    /// Invalid outb port: {0}
    InvalidPort(String),
    /// Failed to read guest log data: {0}
    ReadLogData(String),
    /// Trace formatting error: {0}
    TraceFormat(String),
    /// Failed to read host function call: {0}
    ReadHostFunctionCall(String),
    /// Failed to acquire lock at {0}:{1} - {2}
    LockFailed(&'static str, u32, String),
    /// Failed to write host function response: {0}
    WriteHostFunctionResponse(String),
    /// Invalid character for debug print: {0}
    InvalidDebugPrintChar(u32),
    /// Memory profiling error: {0}
    #[cfg(feature = "mem_profile")]
    MemProfile(String),
}
