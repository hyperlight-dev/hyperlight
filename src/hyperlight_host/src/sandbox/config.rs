...
pub struct SandboxConfiguration {
    ...
    /// size will be determined from the guest binary file header.
    ///
    ...
    /// will be set to the default value of 1000ms. If the guest execution does not complete within
    /// the time specified, then the execution will be cancelled. The minimum value is 1ms.
    ///
    /// Note: this is a C-compatible struct, so even though this optional
    /// field should be represented as an `Option`, that type is not
    /// FFI-safe, so it cannot be.
    ...
    /// execution to be cancelled. If set to 0, the `max_wait_for_cancellation` will be set to the
    /// default value of 10ms. The minimum value is 1ms.
    ///
    /// Note: this is a C-compatible struct, so even though this optional
    /// field should be represented as an `Option`, that type is not
    /// FFI-safe, so it cannot be.
    ...
    // initialize. If set to 0, the `max_initialization_time` will be set to the default value of
    // 2000ms. The minimum value is 1ms.
    //
    // Note: this is a C-compatible struct, so even though this optional
    // field should be represented as an `Option`, that type is not
    // FFI-safe, so it cannot be.
...
impl SandboxConfiguration {
    ...
    // TODO(danbugs:297): arbitrary value, maybe change
