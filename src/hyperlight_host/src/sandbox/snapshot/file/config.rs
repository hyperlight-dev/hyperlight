...
impl OciSnapshotConfig {
    pub(super) fn validate_for_load(&self) -> crate::Result<()> {
        ...
        // region `[BASE_ADDRESS, BASE_ADDRESS + snapshot_size)`. For
        // `Initialise` the address is a GPA and the bound is direct.
        // For `Call` the address is a GVA produced by the guest after
        // init. Hyperlight guests identity-map the snapshot region at
        // low VAs, so the GPA bound also bounds the GVA. A guest that
        // places its dispatch routine at a non-identity-mapped VA
        // will be rejected here and must relax this check.
