...
impl Snapshot {
    ...
    /// supports OCI v1 snapshots with ABI 2. Other format compatibility
    /// can change while Hyperlight is at version 0.x.y. Unsupported
    /// snapshots are rejected during loading.
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// snapshots with ABI 3. Other format compatibility can change
    /// while Hyperlight is at version 0.x.y. Unsupported snapshots are
    ...
    fn load_inner(
        ...
        // Validate the OCI layout marker.
        read_layout_marker(path)?;

        // Resolve `reference` through index.json.
        ...
        // from_file re-stats each handle. Reject a length change since
        // open_snapshot_blob validated the descriptor.
        ...
        // Rebuild the execution state.
        let next_action = NextAction::Call(cfg.entrypoint_addr);

        // Reconstitute host-function metadata.
