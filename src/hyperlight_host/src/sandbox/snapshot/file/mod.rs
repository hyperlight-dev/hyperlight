...
fn decode_transport(bytes: &[u8]) -> crate::Result<VirtqSnapshot> {
    ...
    let reserved = u32::from_le_bytes(bytes[12..16].try_into()?);
    if reserved != 0 {
        return Err(crate::new_error!(
    ...
    let scratch_size = usize::try_from(u64::from_le_bytes(bytes[16..24].try_into()?))?;
    let g2h_len = usize::try_from(u64::from_le_bytes(bytes[24..32].try_into()?))?;
    let h2g_len = usize::try_from(u64::from_le_bytes(bytes[32..40].try_into()?))?;
