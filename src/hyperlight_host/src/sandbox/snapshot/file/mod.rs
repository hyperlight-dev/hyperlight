...
impl Snapshot {
    ...
    /// Like [`Snapshot::from_oci`] but skips the sha256 digest check
    /// on the three content-addressed blobs under
    /// `path/blobs/sha256/`: the OCI image manifest, the Hyperlight
    /// config JSON, and the raw snapshot memory image.
    ///
    /// # Safety
    ///
    /// The caller must guarantee that every blob under `path` is
    /// byte-for-byte the output of a trusted `Snapshot::to_oci`
    /// call.
    pub unsafe fn from_oci_unchecked(path: impl AsRef<Path>, tag: &str) -> crate::Result<Self> {
