...
impl TryFrom<(&X86_64Regs, &MemMgrWrapper<HostSharedMemory>)> for TraceBatch {
    type Error = HyperlightError;
    fn try_from((regs, mem_mgr): (&X86_64Regs, &MemMgrWrapper<HostSharedMemory>)) -> Result<Self> {
...
impl TraceContext {
    ...
    pub fn end_host_trace(&mut self) {
        if self.in_host_call {
            if let Some(entered) = self.host_spans.pop() {
                entered.exit();
            }
        }
    }
