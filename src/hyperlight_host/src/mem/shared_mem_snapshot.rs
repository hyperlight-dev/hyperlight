...
pub(crate) struct SharedMemorySnapshot {
    ...
    /// Size of the sandbox this snapshot was taken of
    sandbox_size: usize,
}
...
impl SharedMemorySnapshot {
    ...
    pub(super) fn new<S: SharedMemory>(
        ...
        Ok(Self {
            ...
            mapped_rgns,
            parent,
            sandbox_size: shared_mem.mem_size(),
        })
    }

    /// Restore shared memory to the state it was in when `self` was taken,
    /// assuming `most_recent_snapshot` is the most recent snapshot `shared_mem` was
    /// in and `current_dirty_pages` are the pages that have been modified since.
    #[instrument(err(Debug), skip_all, parent = Span::current(), level= "Trace")]
    ...
    /// Return the size sandbox this snapshot was taken from.
...
mod tests {
    ...
    fn test_fresh_sandbox_restoration() {
        ...
        let snapshot =
            Arc::new(super::SharedMemorySnapshot::new(&mut gm, &dirty_bitmap, 0, None).unwrap());
    ...
    fn test_rollback_to_ancestor() {
        ...
        let snapshot1 =
            Arc::new(super::SharedMemorySnapshot::new(&mut gm, &[0b1], 0, None).unwrap());
        ...
        let snapshot2 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[0b10], 0, Some(snapshot1.clone())).unwrap(),
        );
        ...
        let snapshot3 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[0b100], 0, Some(snapshot2.clone()))
                .unwrap(),
        );
    ...
    fn test_current_dirty_pages_restoration() {
        ...
        let snapshot = Arc::new(super::SharedMemorySnapshot::new(&mut gm, &[1], 0, None).unwrap());
    ...
    fn test_snapshot_chain_search() {
        ...
        gm.copy_from_slice(&page0_v1, 0).unwrap();
        let snapshot1 = Arc::new(super::SharedMemorySnapshot::new(&mut gm, &[1], 0, None).unwrap());

        // Snapshot 2: page 1 = 'B' (page 0 unchanged, not stored again)
        let page1_v1 = vec![b'B'; PAGE_SIZE_USIZE];
        gm.copy_from_slice(&page1_v1, PAGE_SIZE_USIZE).unwrap();
        let snapshot2 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[2], 0, Some(snapshot1.clone())).unwrap(),
        );
        ...
        let snapshot3 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[1], 0, Some(snapshot2.clone())).unwrap(),
        );
    ...
    fn test_ancestor_relationship() {
        ...
        let snapshot1 = Arc::new(super::SharedMemorySnapshot::new(&mut gm, &[], 0, None).unwrap());
        let snapshot2 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[], 0, Some(snapshot1.clone())).unwrap(),
        );
        let snapshot3 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[], 0, Some(snapshot2.clone())).unwrap(),
        );
    ...
    fn test_different_branches_error() {
        ...
        // Common ancestor
        let ancestor = Arc::new(super::SharedMemorySnapshot::new(&mut gm, &[], 0, None).unwrap());

        // Two different branches
        let branch1 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[], 0, Some(ancestor.clone())).unwrap(),
        );
        let branch2 = Arc::new(
            super::SharedMemorySnapshot::new(&mut gm, &[], 0, Some(ancestor.clone())).unwrap(),
        );
    ...
    #[test]
    fn test_restore_different_size() {
        let mut gm = ExclusiveSharedMemory::new(PAGE_SIZE_USIZE * 2).unwrap();
        gm.copy_from_slice(&[1u8; PAGE_SIZE_USIZE], 0).unwrap();
        let snapshot =
            Arc::new(super::SharedMemorySnapshot::new(&mut gm, &[0b1], 0, None).unwrap());

        let mut bigger_gm = ExclusiveSharedMemory::new(PAGE_SIZE_USIZE * 3).unwrap();

        let result = snapshot.restore_from_snapshot(&mut bigger_gm, &[], &Some(snapshot.clone()));
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Snapshot size does not match current memory size")
        );
    }
