...
impl TestVirtq {
    pub(crate) fn new() -> Self {
        ...
        // SAFETY: The resolved G2H region spans its configured ring within scratch.
        let g2h_layout =
            unsafe { VirtqLayout::from_base(regions.g2h_ring.start, g2h.size()).unwrap() };
        // SAFETY: The resolved H2G region spans its configured ring within scratch.
        let h2g_layout =
            unsafe { VirtqLayout::from_base(regions.h2g_ring.start, h2g.size()).unwrap() };
        ...
        let mut producer = VirtqProducer::new(h2g_layout, mem, HostNotifier, h2g_pool);
        ...
        write_published_arena_gpa(&scratch, layout.get_transport_arena().base_addr()).unwrap();
