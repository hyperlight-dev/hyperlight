...
impl SandboxMemoryManager<HostSharedMemory> {
    ...
    pub(crate) fn write_guest_function_call(&mut self, call: &FunctionCall) -> Result<u32> {
        ...
        let mut ext_vals = ExternalValues::new();
...
mod h2g_tests {
    ...
    fn h2g_manager(case: &TestCase) -> SandboxMemoryManager<HostSharedMemory> {
        #[cfg(not(unshared_snapshot_mem))]
        let shared_mem =
        ...
        manager.h2g_consumer = Some(case.h2g_consumer());
        manager
    }
    ...
    fn rejects_malformed_h2g_buffers() {
        for (len, expected) in [
            ...
            let mut manager = h2g_manager(&case);
    ...
    fn missing_g2h_result_is_fatal() {
        let case = TestCase::new();
        let mut manager = h2g_manager(&case);
        manager.g2h_consumer = Some(case.g2h_consumer());
