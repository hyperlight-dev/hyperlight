...
pub(super) struct SharedMemorySnapshotManager {
    ...
    /// Subsequent snapshots are deltas from the previous state (i.e. only the dirty pages are stored)
    ...
    /// each time we restore from a snapshot, EVEN if the input/output buffers are not explicitly marked dirty by the host.
...
impl SharedMemorySnapshotManager {
    ...
    fn build_snapshot_from_dirty_page_map<S: SharedMemory>(
        ...
        let mut dirty_pages: Vec<usize> = bit_index_iterator(dirty_page_bitmap).collect();
...
mod tests {
    ...
    fn test_multiple_snapshots_and_restores() {
        ...
        snapshot_manager
            .create_new_snapshot(&mut shared_mem, &dirty_pages2, 0)
            .unwrap();
    ...
    fn test_complex_workflow_simulation() {
        ...
        snapshot_manager
            .create_new_snapshot(&mut shared_mem, &dirty_pages1, 0)
            .unwrap();
