...
#[cfg(target_os = "windows")]
impl From<&CommonRegisters> for [(WHV_REGISTER_NAME, Align16<WHV_REGISTER_VALUE>); 18] {
    fn from(regs: &CommonRegisters) -> Self {
        [
            (
                WHvX64RegisterRax,
...
mod tests {
    ...
    fn round_trip_whp_regs() {
        let original = common_regs();
        let whp_regs: [(WHV_REGISTER_NAME, Align16<WHV_REGISTER_VALUE>); 18] = (&original).into();
        let converted: CommonRegisters = whp_regs.as_ref().try_into().unwrap();
        assert_eq!(original, converted);

        // test for duplicate register error handling
        let original = common_regs();
        let mut whp_regs: [(WHV_REGISTER_NAME, Align16<WHV_REGISTER_VALUE>); 18] =
            (&original).into();
