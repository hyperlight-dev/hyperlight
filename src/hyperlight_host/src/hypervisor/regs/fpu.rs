...
#[cfg(target_os = "windows")]
impl From<&CommonFpu> for [(WHV_REGISTER_NAME, Align16<WHV_REGISTER_VALUE>); 26] {
    fn from(fpu: &CommonFpu) -> Self {
        let mut regs: [(WHV_REGISTER_NAME, Align16<WHV_REGISTER_VALUE>); 26] =
            [Default::default(); 26];
        let mut idx = 0;
...
mod tests {
    ...
    fn round_trip_windows_fpu() {
        ...
        let original = sample_common_fpu();
        let windows: [(WHV_REGISTER_NAME, Align16<WHV_REGISTER_VALUE>); 26] = (&original).into();
        let round_tripped = CommonFpu::try_from(windows.as_ref()).unwrap();
        assert_eq!(original, round_tripped);

        // test for duplicate register error handling
        let original = sample_common_fpu();
        let mut windows: [(WHV_REGISTER_NAME, Align16<WHV_REGISTER_VALUE>); 26] =
            (&original).into();
