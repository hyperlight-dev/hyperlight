...
use crate::{HyperlightError, Result, debug, log_then_return};
...
pub trait InterruptHandle: Debug + Send + Sync {
    ...
    /// Increment the generation counter for a new guest function call (Linux only).
    ///
    /// This must be called exactly once at the start of each guest function call,
    /// before any VCPU execution begins. The returned generation number will be
    /// used throughout the entire guest call, even if the VCPU is run multiple
    /// times (due to host function calls, retries, etc.).
    ///
    /// # Returns
    ///
    /// The new generation number assigned to this guest function call.
    ///
    /// # Note
    ///
...
impl LinuxInterruptHandle {
    ...
    // Increment the generation for a new guest function call. Generation will wrap around at `MAX_GENERATION`.
    pub(crate) fn increment_generation(&self) -> u64 {
        self.running
            ...
            .map(|raw| (raw & !Self::RUNNING_BIT) + 1) // Return the NEW generation
            .unwrap_or(1) // If wrapped, return 1
    }

    // set running to true without incrementing generation
    fn set_running_bit(&self) -> u64 {
        self.running
            .fetch_update(Ordering::Release, Ordering::Acquire, |raw| {
                Some(raw | Self::RUNNING_BIT)
            })
            .map(|raw| raw & !Self::RUNNING_BIT) // Return the current generation
            .unwrap_or(0)
    }
