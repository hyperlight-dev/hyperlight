...
mod debug {
    ...
    impl KvmDebug {
        ...
        pub fn set_breakpoints(&mut self, vcpu_fd: &VcpuFd, step: bool) -> Result<()> {
            let addrs: &[u64] = &self.hw_breakpoints;

            if addrs.len() >= Self::MAX_NO_OF_HW_BP {
                return Err(new_error!(
                    "The breakpoints array is larger than expected: {} >= {}",
                    addrs.len(),
                    Self::MAX_NO_OF_HW_BP
                ));
            }
    ...
    impl KVMDriver {
        ...
        pub fn set_single_step(&mut self, enable: bool) -> Result<()> {
            let debug = self
                .debug
                .as_mut()
                .ok_or_else(|| new_error!("Debug is not enabled"))?;

            debug.set_breakpoints(&self.vcpu_fd, enable)
        }
        ...
        pub fn write_regs(&self, regs: &X86_64Regs) -> Result<()> {
...
impl KVMDriver {
    ...
    pub(super) fn new(
        ...
        #[cfg(gdb)]
        let (debug, gdb_conn) = if let Some(DebugInfo { port }) = debug_info {
            (
                Some(debug::KvmDebug::new()),
