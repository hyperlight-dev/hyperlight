...
use crate::hypervisor::virtual_machine::{
    CreateVmError, HypervisorImplError, MapMemoryError, RegisterError, RunVcpuError,
...
impl VirtualMachine for WhpVm {
    unsafe fn map_memory(
        ...
        if res.is_err() {
            return Err(MapMemoryError::Hypervisor(
                HypervisorImplError::WindowsError(windows_result::Error::from_hresult(res)),
            ));
        }
        ...
        WHP_REGS_NAMES
            ...
            .map_err(|e| {
                RegisterError::GetRegs(
                    crate::hypervisor::virtual_machine::HypervisorImplError::WindowsError(
    ...
    fn fpu(&self) -> std::result::Result<CommonFpu, RegisterError> {
        ...
        WHP_FPU_NAMES
            ...
            .try_into()
            .map_err(|e| {
                RegisterError::GetFpu(
                    crate::hypervisor::virtual_machine::HypervisorImplError::WindowsError(
                        windows_result::Error::new(
                            HRESULT::from_win32(160), // ERROR_BAD_ARGUMENTS
                            format!("Failed to convert WHP registers to CommonFpu: {:?}", e),
                        ),
                    ),
                )
            })
