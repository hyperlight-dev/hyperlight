...
mod aarch64;
// Shared imports used by the struct definition and shared methods.
#[cfg(gdb)]
...
// ── Shared helper functions (architecture-independent) ──
...
// ── Error types (architecture-independent) ──
...
// ── Struct definition (shared across architectures) ──
...
pub(crate) struct HyperlightVm {
    ...
    pub(super) entrypoint: NextAction,
    ...
    #[cfg(gdb)]
    pub(super) sw_breakpoints: HashMap<u64, u8>,
    #[cfg(feature = "mem_profile")]
...
// ── Shared methods (architecture-independent) ──
...
impl HyperlightVm {
    ...
    /// The main vCPU execution loop. Runs the vCPU until halt, cancellation, or error.
    pub(super) fn run(
        &mut self,
        ...
        match result {
            Ok(_) => Ok(()),
            Err(RunVmError::ExecutionCancelledByHost) => Err(RunVmError::ExecutionCancelledByHost),
            Err(e) => {
                #[cfg(crashdump)]
