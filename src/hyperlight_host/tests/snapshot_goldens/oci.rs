...
//! On-disk cache lookup for golden OCI Image Layouts.

use std::path::PathBuf;

use crate::platform::GOLDENS_VERSION;
...
pub fn cache_root() -> PathBuf {
    ...
    target.join("snapshot-goldens-cache").join(GOLDENS_VERSION)
}

fn cache_dir_for(tag: &str) -> PathBuf {
    cache_root().join(tag)
}

/// Locate the golden OCI Image Layout for `tag` in the local
/// cache. A missing layout is an error with guidance to populate
/// the cache.
pub fn golden_dir(tag: &str) -> Result<PathBuf, String> {
    let dir = cache_dir_for(tag);
