...
/// Get a fully qualified OS-specific path to the customguest elf binary
pub fn custom_guest_as_string() -> Result<String> {
    let buf = rust_guest_as_pathbuf("customguest");
    buf.to_str()
        .map(|s| s.to_string())
        .ok_or_else(|| anyhow!("couldn't convert custom guest PathBuf to string"))
}
