...
struct ScratchTopMetadata {
    /// Keep the exception stack at its aligned ABI offset.
...
const _: () = {
    ...
    assert!(SCRATCH_TOP_G2H_RING_GPA_OFFSET == 0x40);
    assert!(SCRATCH_TOP_G2H_POOL_GPA_OFFSET == 0x48);
    assert!(SCRATCH_TOP_G2H_POOL_PAGES_OFFSET == 0x50);
    assert!(SCRATCH_TOP_G2H_BUFFER_SIZE_OFFSET == 0x58);
    assert!(SCRATCH_TOP_H2G_QUEUE_DEPTH_OFFSET == 0x60);
    assert!(SCRATCH_TOP_H2G_RING_GPA_OFFSET == 0x68);
    assert!(SCRATCH_TOP_H2G_POOL_GPA_OFFSET == 0x70);
    assert!(SCRATCH_TOP_H2G_POOL_PAGES_OFFSET == 0x78);
    assert!(SCRATCH_TOP_H2G_BUFFER_SIZE_OFFSET == 0x80);
    assert!(SCRATCH_TOP_EXN_STACK_OFFSET == 0x90);
};
