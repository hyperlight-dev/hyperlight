...
pub trait BufferProvider: Send + Sync + 'static {
...
impl<P: BufferProvider, M: PhysMem> Drop for BufferOwner<P, M> {
    fn drop(&mut self) {
        let _ = self.pool.dealloc(self.alloc);
    }
}

impl<P: BufferProvider, M: PhysMem> AsRef<[u8]> for BufferOwner<P, M> {
    fn as_ref(&self) -> &[u8] {
        todo!()
    }
}
