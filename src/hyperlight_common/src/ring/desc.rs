...
#[inline]
fn read_desc_acquire(base: NonNull<Descriptor>) -> Descriptor {
...
#[inline]
fn write_desc_release(base: NonNull<Descriptor>, desc: &Descriptor) {
    let base_ptr = base.as_ptr().cast::<u8>();
...
impl<'t> DescView<'t> {
    ...
    ///
    /// Returns a plain Descriptor value.
    #[inline]
    pub fn read(&self) -> Descriptor {
        read_desc_acquire(self.base)
    }

    /// Read descriptor from memory without any memory ordering guarantees.
    #[inline]
    pub fn read_fast(&self) -> Descriptor {
        // SAFETY: base is valid descriptor pointer
        unsafe { ptr::read_volatile(self.base.as_ptr()) }
    }
}

/// A mutable view into a Descriptor stored in shared memory.
#[derive(Debug)]
pub struct DescViewMut<'t> {
    base: NonNull<Descriptor>,
    owner: PhantomData<&'t mut DescTable<'t>>,
}

impl<'t> DescViewMut<'t> {
    /// # Safety: base must be valid for reads/writes for 't
    pub unsafe fn new(base: NonNull<Descriptor>) -> Self {
        assert!((base.as_ptr() as usize) % Descriptor::ALIGN == 0);
    ...
    #[inline]
    pub fn write(&mut self, desc: &Descriptor) {
        write_desc_release(self.base, desc);
    }

    /// Write descriptor to memory without any memory ordering guarantees.
    #[inline]
    pub fn write_fast(&mut self, desc: &Descriptor) {
        // SAFETY: base is valid descriptor pointer
        unsafe { ptr::write_volatile(self.base.as_ptr(), *desc) };
    }

    /// Get an immutable view to the same descriptor
    pub fn as_view(&self) -> DescView<'_> {
        // SAFETY: creating a read-only view to the same location, it cannot outlive &self
        unsafe { DescView::new(self.base) }
    }
}
...
impl<'t> DescTable<'t> {
    ...
    /// Get mutable descriptor at index or None if idx is out of bounds
    pub fn get(&self, idx: u16) -> Option<DescView<'_>> {
        ...
        // SAFETY: raw is valid
        Some(unsafe { DescView::new(raw) })
    }

    /// Get immutable descriptor at index or None if idx is out of bounds
    pub fn get_mut(&mut self, idx: u16) -> Option<DescViewMut<'_>> {
        if idx >= self.size as u16 {
            return None;
        }
        ...
        let raw = unsafe { NonNull::new_unchecked(base.add(idx as usize)) };
        // SAFETY: raw is valid
        Some(unsafe { DescViewMut::new(raw) })
    }
