...
impl Display for AllocError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            AllocError::InvalidAlign(a) => write!(f, "Invalid region addr {a}"),
            AllocError::InvalidFree(a, s) => write!(f, "Invalid free addr {a} and size {s}"),
            AllocError::InvalidArg => write!(f, "Invalid argument"),
            AllocError::EmptyRegion => write!(f, "Empty region"),
            AllocError::OutOfMemory => write!(f, "Out of memory"),
            AllocError::Overflow => write!(f, "Overflow"),
        }
    }
}
...
impl<const N: usize> Slab<N> {
    ...
    pub fn find_slots(&mut self, slots_num: usize) -> Option<usize> {
        ...
        for next_free in self.used_slots.zeroes() {
            if self
                .used_slots
    ...
    pub fn free(&mut self, alloc: Allocation) -> Result<(), AllocError> {
...
impl<const L: usize, const U: usize> BufferPool<L, U> {
    ...
    pub fn free(&mut self, alloc: Allocation) -> Result<(), AllocError> {
        if self.lower.contains(alloc.addr) {
            self.lower.free(alloc)
        } else {
            self.upper.free(alloc)
        }
    }
...
fn maybe_move<const A: usize, const B: usize>(
    ...
    if !needs_move {
        return slab.realloc(old_alloc, new_len);
    }
...
mod tests {
    ...
    fn test_slab_realloc_grow_inplace() {
        ...
        let new_alloc = slab.realloc(alloc, 512).unwrap();
    ...
    fn test_slab_realloc_grow_relocate() {
        ...
        let new_alloc = slab.realloc(a1, 512).unwrap();
    ...
    fn test_slab_realloc_shrink() {
        ...
        let new_alloc = slab.realloc(alloc, 256).unwrap();
    ...
    fn test_pool_new_success() {
        ...
        assert!(pool.lower.capacity() > 0);
        assert!(pool.upper.capacity() > 0);
    }

    #[test]
    fn test_pool_alloc_small_to_lower() {
        let mut pool = make_pool::<256, 4096>(1024 * 1024);
        let alloc = pool.alloc(128).unwrap();

        // Should come from lower slab
        assert!(pool.lower.contains(alloc.addr));
        assert_eq!(alloc.len, 256);
    }
    ...
    fn test_pool_alloc_large_to_upper() {
        ...
        assert!(pool.upper.contains(alloc.addr));
    ...
    fn test_pool_alloc_fallback_to_upper() {
        ...
        while pool.lower.free_bytes() > 0 {
            allocations.push(pool.lower.alloc(256).unwrap());
        }
    ...
    fn test_pool_free_from_lower() {
        ...
        let free_before = pool.lower.free_bytes();
        pool.free(alloc).unwrap();
        assert_eq!(pool.lower.free_bytes(), free_before + alloc.len);
    }
    ...
    fn test_pool_free_from_upper() {
        ...
        let free_before = pool.upper.free_bytes();
        pool.free(alloc).unwrap();
        assert_eq!(pool.upper.free_bytes(), free_before + alloc.len);
    }
    ...
    fn test_pool_realloc_within_same_tier() {
        ...
        let new_alloc = pool.realloc(alloc, 200).unwrap();
        assert!(pool.lower.contains(new_alloc.addr));
    }
    ...
    fn test_pool_realloc_move_to_different_tier() {
        ...
        let new_alloc = pool.realloc(alloc, 1500).unwrap();
        assert!(pool.upper.contains(new_alloc.addr));
    }
    ...
    fn test_pool_realloc_shrink_stays_in_tier() {
        ...
        let new_alloc = pool.realloc(alloc, 1000).unwrap();
        assert!(pool.upper.contains(new_alloc.addr));
    }

    #[test]
    fn test_pool_stress_many_allocations() {
        let mut pool = make_pool::<256, 4096>(4 * 1024 * 1024);
        let mut allocations = Vec::new();
        ...
        for i in (0..100).step_by(2) {
            pool.free(allocations[i]).unwrap();
        }
    ...
    fn test_pool_mixed_workload() {
        let mut pool = make_pool::<256, 4096>(2 * 1024 * 1024);
        ...
        // Free and reallocate
        pool.free(rx_buf1).unwrap();
        let rx_buf3 = pool.alloc(1500).unwrap();

        // Should reuse freed buffer (LIFO)
        assert_eq!(rx_buf3.addr, rx_buf1.addr);

        pool.free(desc_buf).unwrap();
        pool.free(rx_buf2).unwrap();
        pool.free(rx_buf3).unwrap();
        pool.free(tx_buf).unwrap();
    }
    ...
    fn test_pool_boundary_allocation() {
        ...
        assert!(pool.upper.contains(alloc2.addr));
    }
...
mod quick {
    ...
    fn run_scenario(s: Scenario) -> bool {
        ...
        let mut pool = match BufferPool::<256, 4096>::new(base, s.pool_size) {
            Ok(p) => p,
            Err(_) => return true,
        };
        ...
        for op in &s.ops {
            match op {
                ...
                Op::Realloc(idx, new_size) => {
                    ...
                    match pool.realloc(old_alloc, *new_size) {
                        Ok(new_alloc) => {
    ...
    fn check_pool_invariants<const L: usize, const U: usize>(
        ...
        check_slab_invariants(&pool.lower)?;
        check_slab_invariants(&pool.upper)?;

        if pool.lower.range().end > pool.upper.range().start {
        ...
        for alloc in allocations {
            if !pool.lower.contains(alloc.addr) && !pool.upper.contains(alloc.addr) {
