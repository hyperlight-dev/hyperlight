...
impl<'t> RingProducer<'t> {
    pub fn new(table: DescTable<'t>) -> Self {
        let size = table.len();
...
impl<'t> RingConsumer<'t> {
    ...
    pub fn poll_available(&mut self) -> Result<(u16, BufferChain), RingError> {
        ...
        let head_view = self.desc_table.get(idx).ok_or(RingError::InvalidState)?;
        // Acquire: flags then fields (publish point)
        let head_desc = head_view.read();
...
mod tests {
    ...
    fn test_poll_multi_descriptor_chain() {
        ...
        let mut head_desc = head_view.read();
    ...
    fn test_tail_marked_used_ignored() {
        ...
        let mut tail_desc = tail_view.read();
        tail_desc.mark_used(producer.used_cursor.wrap());
        tail_view.write(&tail_desc);
    ...
    fn bad_driver_next_never_clears() {
        ...
        for _ in 0..size {
            ...
            desc.mark_avail(pos.wrap());
            prod.desc_table.get_mut(idx).unwrap().write_fast(&desc);
            pos.advance();
        }
        ...
        prod.desc_table.get_mut(head_idx).unwrap().write(&head_desc);
