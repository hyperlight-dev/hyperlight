...
use hyperlight_common::ring::BufferPool;
...
fn bench_realloc(c: &mut Criterion) {
    ...
    group.bench_function("grow_inplace", |b| {
        ...
        b.iter(|| {
            ...
            let grown = pool.realloc(alloc, black_box(512)).unwrap();
    ...
    group.bench_function("grow_relocate", |b| {
        ...
        b.iter(|| {
            ...
            let grown = pool.realloc(alloc, black_box(1500)).unwrap();
            pool.free(grown).unwrap();
            pool.free(blocker).unwrap();
        });
    });
    ...
    group.bench_function("shrink", |b| {
        ...
        b.iter(|| {
            ...
            let shrunk = pool.realloc(alloc, black_box(256)).unwrap();
    ...
    for size in [256, 1500, 4096].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            ...
            b.iter(|| {
                ...
                pool.free(black_box(alloc)).unwrap();
            });
        });
    }
