...
use std::rc::Rc;
use std::sync::Mutex;
use piet_common::{Color, kurbo, RenderContext, Text, TextLayoutBuilder, TextLayout};
...
impl<T: Default> TraceTrie<T> {
    ...
    fn apply_path<'a, 'i, F: Fn(&mut T), I: Iterator<Item = &'i u64>>(&'a mut self, trace: I, f: F) {
        let mut node = self;
        for frame in trace {
            f(&mut node.value);
            node = (*node).children
    ...
    fn resolve_symbol<'c>(&'c mut self, addr: u64)  -> &'c Option<(String, Option<u32>)> {
        self.symbol_cache.entry(addr).or_insert_with(|| {
            ...
            let demangled = addr2line::demangle_auto(
                function.name.to_string_lossy(), function.language
            ).to_string();
...
fn draw_bar<R: piet_common::RenderContext>(
    ...
    if stroke {
        render_context.stroke(kurbo::Rect {
            x0: left,
            y0: top,
            //x1: v.margin + v.width,
    ...
    let layout = render_context.text()
        .new_text_layout(label)
        .build().ok()?;
    let metric = layout.line_metric(0)
        .expect("there must be at least one line metric");
    render_context.draw_text(&layout, kurbo::Point {
    ...
    let layout = render_context.text()
        ...
        .default_attribute(piet_common::TextAttribute::TextColor(Color::rgb8(255, 255, 255)))
        .build().ok()?;
    let metric = layout.line_metric(0)
        .expect("there must be at least one line metric");
...
fn render_bitmap<F: RenderWrapper, O: Write>(
    mut out: O, device: &mut piet_common::Device, mut render: F
) -> Option<()> {
    let width = 1920;
    let height = 1080;
    ...
    out.write_all(
        bitmap.to_image_buf(piet_common::ImageFormat::RgbaPremul).expect("unable to access image buffer").raw_pixels()
...
impl<'r, 'a, 's> RenderWrapper for BarRenderer<'r> {
    fn render<R: RenderContext>(
        &mut self, ctx: &mut R, wd: u64, ht: u64
    ) -> Option<()> {
        let v = ViewParams::new(ctx, ht, wd);
        draw_bg(ctx, &v);
        draw_bar(ctx, &v,
            true, 0, "Execution time".to_string(),
            self.now.as_micros() as u64,
            self.state.max_duration.as_micros() as u64)?;
        draw_bar(ctx, &v,
            true, 1, "Total memory consumption".to_string(),
            self.state.total, self.state.max_total)?;
...
fn draw_flame<R: RenderContext>(
    ...
    if let Some(addr) = addr {
        ...
        let layout = ctx.text()
            ...
            .build().ok()?;
        ctx.draw_text(&layout, kurbo::Point {
            x: v.margin + fv.left,
...
impl<'r, 'a, 's> RenderWrapper for FlameRenderer<'r> {
    fn render<R: RenderContext>(
        ...
        draw_flame(ctx, &v, &fv, &mut self.state.symbol_cache, &self.state.traces, None)?;
        Some(())
    }
}
...
fn render_free(
    ...
    let (amt, trace) = state.allocs.get(&ptr).expect("free of un-allocated address");
    for frame in trace.as_ref() {
        *state.sites.get_mut(frame).expect("free of un-allocated site") -= amt;
    }
    state.traces.apply_path(trace.iter().rev(), |t| *t -= amt);
    render_state(state, rs, now)?;
    Some(())
 ...
 F: Fn(&mut State, &mut S, Duration, u64, u64, Rc<[u64]>) -> Option<()>,
  {
    loop {
        ...
        let now = Duration::from_micros(
            time
                .try_into()
                .expect("duration too large for u64")
        );
        ...
        if frame_id == 0 {
            ...
            let amt_trace =
                state.allocs.get(&ptr).expect("free of un-allocated address");
            let amt = amt_trace.0;
...
fn ffmpeg_for(
    out_dir: &PathBuf, vis: Visualisation, start: Duration
) -> Option<std::process::Child> {
    let out = std::fs::File::create(
        out_dir.join(format!("{:08}.{}.out", start.as_micros(), vis))
    ).ok()?;
    let err = std::fs::File::create(
        out_dir.join(format!("{:08}.{}.err", start.as_micros(), vis))
    ).ok()?;
    let mkv = mkv_for(out_dir, vis, start);
    let _ = std::fs::remove_file(&mkv);
    std::process::Command::new("ffmpeg")
        .args([
            "-f", "rawvideo", "-pix_fmt", "rgba", "-framerate", "60",
            "-video_size", "1920x1080", "-i", "-",
            "-c:v", "libvpx-vp9", "-crf", "15", "-b:v", "0"])
        .arg(mkv)
...
fn spawn_render_thread(
    ...
    std::thread::spawn(move || {
        let out_dir = out_dir?;
        eprintln!("> {:08} -- {:08}",
            interval.0.as_micros(), interval.1.as_micros());
        ...
        let mut device = piet_common::Device::new()
            .expect("could not create Piet device");
        let rs = RenderState {
            device: &mut device,
            bar_out: bar_ffmpeg.stdin.take().expect("bar ffmpeg stdin"),
            flame_out: flame_ffmpeg.stdin.take().expect("flame ffmpeg stdin"),
        };
        read_file(&mut job_state, rs, render_ident, render_unwind, render_alloc, render_free)?;
        bar_ffmpeg.wait().ok()?;
...
fn main() {
    ...
    if !is_list && !is_plot {
        ...
        eprintln!("usage: {} <guest bin> <trace file> plot_mem <out dir> <jobs>", args[0]);
        return;
    }
...
fn plot_mem(args: Vec<String>, mut state: State) {
    ...
    match read_file(&mut state,
        ...
        |_, _, _, _, _, _| Some(())) {
            Some(()) => (),
            None => { eprintln!("i/o error encountered"); ()
            }
    }
    eprintln!("max total memory used is {}", state.max_total);
    state.inf.seek(SeekFrom::Start(0)).expect("couldn't seek back");
    state.allocs = HashMap::new();
    state.total = 0;

    /* second pass: compute fair durations so that each parallel job
     * processes the same number of frames */
    let num_segments = str::parse::<u64>(&args[5])
        .expect("number of segments must be a number");
    ...
    if state.num_durations > 0 {
        (*jobs.lock().unwrap()).push((*start_duration.lock().unwrap(),
            state.max_duration));
    }
    ...
    for job in &*jobs.lock().unwrap() {
        handles.push(spawn_render_thread(&mut state, args[1].clone(), args[2].clone(), *job));
    }
    ...
    for (n, job) in (*jobs.lock().unwrap()).iter().enumerate() {
        if n > 0 { merge_bar.arg("+"); merge_flame.arg("+"); }
