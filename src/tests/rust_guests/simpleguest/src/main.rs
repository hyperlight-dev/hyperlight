...
fn read_mapped_buffer(base: u64, len: u64, do_map: bool) -> Vec<u8> {
    ...
    if do_map {
        // Callers pass a page-aligned base and a nonzero length, so rounding
        // the length up covers the whole buffer.
        unsafe {
...
fn write_mapped_buffer(base: u64, len: u64) -> bool {
    ...
    // Callers pass a page-aligned base and a nonzero length, so rounding the
    // length up covers the whole buffer.
    unsafe {
        hyperlight_guest_bin::paging::map_region(
            base as _,
            base as _,
