...
* **config** (`application/vnd.hyperlight.snapshot.config.v1+json`). The
  snapshot descriptor: arch, hypervisor, ABI version, entrypoint,
  memory layout, registered host functions, snapshot generation
  counter. Loaded eagerly and fully parsed.
...
and snapshot blobs. `Snapshot::from_oci_unchecked` is `unsafe` and
skips digest verification, trading integrity for performance. It
keeps every other check (OCI structure, descriptor sizes, schema
